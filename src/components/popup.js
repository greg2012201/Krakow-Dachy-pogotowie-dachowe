import {getElementPosition} from '../tools/getElementPosition';
...
const getPopupLegHeight = () => {
  ...
  return (
    Math.hypot(
      getElementPosition(square).height,
      getElementPosition(square).width
    ) / 2
  );
};
...
const getDirection = () => {
  ...
  return {
    ...
    isEnouthSpaceOnBottom:
      innerHeight >=
      linkPosition.bottom + popupPosition.height + popupLegHeight,
    isEnouthSpaceOnTop:
      innerHeight <= linkPosition.top + popupPosition.height + popupLegHeight,
  };
};
...
const dispatch = (events) => {
  ...
  const {up, down, isEnouthSpaceOnBottom, isEnouthSpaceOnTop} = directions;
  const {eventUp, eventDown} = events;
...
const createEvent = () => {
  ...
  dispatch(events);
  RAF = requestAnimationFrame(createEvent);
};
